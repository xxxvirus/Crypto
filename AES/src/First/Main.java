...
public class Main {
	...
	public static void main(String[] args) {
		...
		int[][] stateWK = new int[4][4];
		int[][] stateWKIV = new int[4][4];
		int[][] stateWKIVf = new int[4][4];
		int[][] stateWKIVfin = new int[4][4];
		int[][] stateWKin = new int[4][4];
		int[][] stateKin = new int[4][4];
		System.out.println("KeyState");
		for (int i = 0, k = 0; i < stateK.length; i++) {
			for (int j = 0; j < stateK.length; j++) {
				stateK[i][j] = keyB[k++];
				System.out.print(" " + stateK[i][j]);
			}
			System.out.println(" ");
		}
		...
		for (int i = 0; i < stateWK.length; i++) {
			for (int j = 0; j < stateWK.length; j++) {
				stateWK[i][j] = state[i][j] + stateK[i][j];
				stateWK[i][j] = stateWK[i][j] % 256;
				System.out.print(" "+stateWK[i][j]);
			}
			System.out.println(" ");
		}
		System.out.println("State KeyIV");
		for (int i = 0, k=0; i < stateWKIV.length; i++) {
			for (int j = 0; j < stateWKIV.length; j++) {
				stateWKIV[i][j] = key4[k++];
				System.out.print(" " + stateWKIV[i][j]);
			}
			System.out.println(" ");
		}
		System.out.println("State + Key + KeyIV");
		for (int i = 0; i < stateWKIV.length; i++) {
			for (int j = 0; j < stateWKIV.length; j++) {
				stateWKIVf[i][j] = stateWKIV[i][j] + stateWK[i][j];
				stateWKIVf[i][j] = stateWKIVf[i][j] % 256;
				System.out.print(" " + stateWKIVf[i][j]);
			}
			System.out.println(" ");
		}
		System.out.println("SubBytes ");
		for (int i = 0; i < stateWKIVf.length; i++) {
			for (int j = 0; j < stateWKIVf[0].length; j++) {
				int a = stateWKIVf[i][j];
				stateWKIVf[i][j] = sbox[a / 16][a % 16];
				stateWKIVf[i][j] = stateWKIVf[i][j] % 256;
				System.out.print(" " + stateWKIVf[i][j]);
			}
			System.out.println(" ");
		}
		System.out.println("SubBytesRev");
		for (int i = 0; i < stateWKIVfin.length; i++) {
			for (int j = 0; j < stateWKIVfin.length; j++) {
				int a = stateWKIVf[i][j];
				stateWKIVfin[i][j] = invsbox[a / 16][a % 16];
				stateWKIVfin[i][j] = stateWKIVfin[i][j] % 256;
				System.out.print(" " + stateWKIVfin[i][j]);
			}
			System.out.println(" ");
		}
		System.out.println("- KeyIV");
		for (int i = 0; i < stateWKin.length; i++) {
			for (int j = 0; j < stateWKin.length; j++) {
				stateWKin[i][j] = stateWKIVfin[i][j] - stateWKIV[i][j];
				if(stateWKin[i][j]>0){
					stateWKin[i][j] = stateWKin[i][j] % 256;
				}else{
					stateWKin[i][j] = stateWKin[i][j] + 256;
				}
				System.out.print(" " + stateWKin[i][j]);
			}
			System.out.println(" ");
		}
		System.out.println("- Key");
		for (int i = 0; i < stateKin.length; i++) {
			for (int j = 0; j < stateKin.length; j++) {
				stateKin[i][j] = stateWKin[i][j] - stateK[i][j];
				if(stateKin[i][j]>=0){
					stateKin[i][j] = stateKin[i][j] % 256;
				}else{
					stateKin[i][j] = stateKin[i][j] + 256;
				}
				System.out.print(" " + stateKin[i][j]);
			}
			System.out.println(" ");
		}
		...
		// }
		/*
		 * for (int i = 0; i < en.length; i++) { int hex = en[i]; en[i] =
		 * S[hex]; System.out.println(en[i]); }
		 */
